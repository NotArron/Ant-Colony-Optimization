...
ant_colony = AntColonyOptimization(cities, totalAnt=47, alpha=1.0, beta=2.0, evaporation=0.1)

bestPath, bestPathLength = ant_colony.run(iterasi=100)
...
# Display the image
plt.imshow(image, extent=[min(x_coordinate)*1.15, max(x_coordinate)*1.07, min(y_coordinate)*1.18, max(y_coordinate)*1.35])
